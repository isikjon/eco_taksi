...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _initializeWebSocket() async {
    try {
      ...
      if (clientData != null) {
        _currentDriverId = clientData['id'];
        _currentTaxiparkId = clientData['taxiparkId'];
  ...
  Future<void> _loadDriverData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _driverData = clientData;
          // Загружаем способ оплаты из БД
          if (clientData != null && clientData['preferred_payment_method'] != null) {
            final paymentMethod = clientData['preferred_payment_method'];
            _selectedPaymentMethod = paymentMethod == 'cash' ? 'наличными' : paymentMethod;
          }
        });
      }
      
      // Загружаем данные таксопарка и тариф
      if (clientData != null && clientData['phoneNumber'] != null) {
        final taxiparkData = await DriverService().getDriverTaxipark(clientData['phoneNumber']);
        final driverProfile = await DriverService().getDriverProfile(clientData['phoneNumber']);
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildMenuRow('Оплата', _selectedPaymentMethod, onTap: () async {
            print('Нажали на Оплата');
            try {
              final result = await Navigator.of(context).push(
                MaterialPageRoute(
              ...
              print('Результат от PaymentMethodScreen: $result');
              // Обновляем выбранный метод оплаты если он изменился
              ...
              print('Ошибка при переходе к PaymentMethodScreen: $e');
            }
          }),
          ...
          _buildMenuRow('Безопасность', null, onTap: () {
            Navigator.of(context).push(
              MaterialPageRoute(
